            ...
            if (gameState.enemy.currentHealth <= 0) {
                ...
                if (isBossBattle) {
                    ...
                    if (Math.random() < 0.3) {
                        ...
                        addToBattleLog(`Found ${randomItem.name}!`, 'text-blue-400');
                    }

                    addToBattleLog(`Received ${goldGain} gold and ${expGain} exp!`);
        ...
        function showVictoryScreen() {
            ...
            if (gameState.enemy.bossMechanics && gameState.enemy.bossMechanics.onDefeat) {
                ...
                elements.victoryMessage.textContent += `\nReceived ${goldGain} gold, ${expGain} exp and ${randomItem.name}!`;
            }
        ...
        function resetGame() {
            ...
            gameState.player.currentHealth = 100;
        ...
        function applyDebuff(gameState) {
            if (!gameState.battleActive || !gameState.enemy || gameState.enemy.id !== 'shadow_wraith') return;

            const debuffChance = 0.7; // 70% chance to apply debuff
            if (Math.random() < debuffChance) {
                const debuffAmount = 0.2; // 20% reduction in damage
                const debuffDuration = 3; // Debuff lasts for 3 turns

                // Check for debuff resistance
                const debuffResistance = gameState.player.activeBuffs.find(buff => buff.stat === 'debuffResistance');
                let resisted = false;
                if (debuffResistance && Math.random() < debuffResistance.amount) {
                    resisted = true;
                    addToBattleLog("Debuff resisted!", 'text-green-400');
                }
                ...
                if (!resisted) {
                    gameState.player.activeBuffs.push({
                        ...
                        type: 'debuff',
                        stat: 'damage',
                        amount: -debuffAmount, // Negative amount for debuff
                        duration: debuffDuration,
                        icon: 'fa-curse',
                        name: 'Weakened'
                    });
                    renderActiveBuffs();
                    addToBattleLog("Weakened! Damage reduced by 20% for 3 turns.", 'text-red-400');
                } else {
                    addToBattleLog("Debuff resisted!", 'text-green-400');
                }
            }
